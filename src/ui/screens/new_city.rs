...
pub fn render_new_city(frame: &mut Frame, area: Rect, view: &NewCityViewModel, state: &mut NewCityState) {
    ...
    frame.render_widget(MapPreview { map: &view.preview_map }, inner_map);
...
fn render_controls(frame: &mut Frame, area: Rect, view: &NewCityViewModel, state: &mut NewCityState) {
    ...
    let draw_field = |buf: &mut ratatui::buffer::Buffer, rect: Rect, text: &str, focused: bool, fg: ratatui::style::Color, bg: ratatui::style::Color| {
...

    {
        ...
        buf.set_string(area.x, row, "Seed (hex):", Style::default().fg(ui.text_secondary));
        row += 1;
...

    {
        ...
        buf.set_string(area.x, row, format!("Water: {}%", view.water_pct), Style::default().fg(ui.text_secondary));
        row += 1;
        ...
        render_bar(buf, Rect::new(area.x, row, w, 1), view.water_pct, ui.slider_water_fg, if is_focused { ui.slider_water_focus_bg } else { ui.slider_bg }, ui);
        row += 2;
    }

    {
        let is_focused = view.focused_field == NewCityField::TreesSlider;
        buf.set_string(area.x, row, format!("Trees: {}%", view.trees_pct), Style::default().fg(ui.text_secondary));
        row += 1;
        ...
        render_bar(buf, Rect::new(area.x, row, w, 1), view.trees_pct, ui.slider_trees_fg, if is_focused { ui.slider_trees_focus_bg } else { ui.slider_bg }, ui);
        row += 2;
    }
    ...
    for (field, text) in [
        ...
        buf.set_string(area.x, row, format!("{:<width$}", text, width = w as usize), style);
        row += 2;
    }
    ...
    if row < area.y + area.height {
        ...
        buf.set_string(area.x, area.y + area.height - 1, &trimmed, Style::default().fg(ui.text_dim));
    }
}
