...
impl RectDrag {
    pub fn new(tool: Tool, x: usize, y: usize) -> Self {
        Self { tool, start_x: x, start_y: y, end_x: x, end_y: y, tiles_cache: vec![(x, y)] }
    }

    pub fn update_end(&mut self, ex: usize, ey: usize) {
        if self.end_x == ex && self.end_y == ey { return; }
        self.end_x = ex;
        self.end_y = ey;
        ...
        for y in y0..=y1 { for x in x0..=x1 { self.tiles_cache.push((x, y)); } }
    }

    pub fn width(&self)  -> usize { self.start_x.abs_diff(self.end_x) + 1 }
    pub fn height(&self) -> usize { self.start_y.abs_diff(self.end_y) + 1 }
}
