    ...
    fn tick(&mut self, map: &mut Map, sim: &mut SimState) {
        ...
        for (cx, cy, _consumption) in consumer_idxs {
            ...
            for (nx, ny, tile) in map.neighbors4(cx, cy) {
                if tile.power_connects() || tile == Tile::PowerPlantCoal || tile == Tile::PowerPlantGas {
                    let p = map.get_overlay(nx, ny).power_level;
                    if p > max_adj_power { max_adj_power = p; }
                }
            }
            if max_adj_power > 5 { // Threshold to be "powered"
...
impl SimSystem for FinanceSystem {
    ...
    fn tick(&mut self, map: &mut Map, sim: &mut SimState) {
        ...
        let residential_tax = annual_tax_from_base(sim.residential_population, sim.tax_rates.residential);
        ...
        sim.demand_res  = (ideal_res  - current_res_ratio  + res_tax_modifier + growth_boost).clamp(-1.0, 1.0);
        sim.demand_comm = (ideal_comm - current_comm_ratio + comm_tax_modifier + growth_boost).clamp(-1.0, 1.0);
        sim.demand_ind  = (ideal_ind  - current_ind_ratio  + ind_tax_modifier + growth_boost).clamp(-1.0, 1.0);
    }
}
...
mod tests {
    ...
    fn finance_sector_taxes_sum_to_total_tax() {
        ...
        let b = &sim.last_breakdown;
        assert_eq!(b.annual_tax, b.residential_tax + b.commercial_tax + b.industrial_tax);
    ...
    fn power_decay_over_distance() {
        ...
        for dy in 0..4 { for dx in 0..4 { map.set(dx, dy, Tile::PowerPlantCoal); } }
        
        // Place a long line of power lines
        for x in 4..15 { map.set(x, 0, Tile::PowerLine); }
        ...
        assert!(level_near > level_far, "Power level should decay over distance ({} > {})", level_near, level_far);
        assert!(level_far > 0, "Power should still reach the end of the line");
    }
    ...
    fn power_plant_expiration_and_rubble() {
        ...
        assert_eq!(map.get(0, 0), Tile::Rubble, "Tile should be replaced with Rubble");
    }
    ...
    fn power_brownout_scaling() {
        ...
        assert!(level < 200, "Power level should be scaled down during brownout (got {})", level);
    }
}
