use crate::core::engine::{SimulationEngine, EngineCommand};
...
mod tests {
    ...
    fn test_place_single_tool() {
        ...
        // Place a road
        let cmd = EngineCommand::PlaceTool { tool: Tool::Road, x: 5, y: 5 };
        let result = engine.execute_command(cmd);

        assert!(result.is_ok(), "Tool placement should succeed");
        assert_eq!(engine.map.get(5, 5), Tile::Road, "Tile should be updated to Road");
        assert_eq!(engine.sim.treasury, initial_funds - cost, "Treasury should be deducted by tool cost");
    }
    ...
    fn test_place_tool_out_of_bounds() {
        ...
        let cmd = EngineCommand::PlaceTool { tool: Tool::Road, x: 10, y: 10 };
        let result = engine.execute_command(cmd);

        assert!(result.is_err(), "Tool placement out of bounds should fail");
        assert_eq!(engine.sim.treasury, initial_funds, "Treasury should not change on failure");
    }
    ...
    fn test_insufficient_funds() {
        ...
        assert_eq!(engine.sim.treasury, 10, "Treasury should not change on failure");
    }
    ...
    fn test_place_line() {
        ...
        let cmd = EngineCommand::PlaceLine { tool: Tool::Road, path: path.clone() };
        let result = engine.execute_command(cmd);
        ...
        assert_eq!(engine.sim.treasury, initial_funds - (cost * 3), "Treasury should be deducted for 3 tiles");
    }
    ...
    fn test_place_line_with_obstacle() {
        ...
        // By design (in tuicity2), line drag filters out invalid tiles *before* committing,
        // but `place_line` iterates and skips `!tool.can_place(existing)`.
        let result = engine.execute_command(cmd);

        assert!(result.is_ok(), "Line placement should succeed (skipping invalid tiles)");
        assert_eq!(engine.map.get(1, 1), Tile::Road, "Valid tile should be placed");
        assert_eq!(engine.map.get(1, 2), Tile::Water, "Water should remain blocking");
        assert_eq!(engine.map.get(1, 3), Tile::Road, "Valid tile should be placed");
    }
    ...
    fn test_place_rect() {
        ...
        let cmd = EngineCommand::PlaceRect { tool: Tool::ZoneRes, tiles };
        let result = engine.execute_command(cmd);
        ...
        assert_eq!(engine.sim.treasury, initial_funds - (cost * 4), "Treasury should be deducted for 4 tiles");
    }
    ...
    fn test_road_powerline_intersection() {
        ...
        // Place a road
        let cmd1 = EngineCommand::PlaceTool { tool: Tool::Road, x: 5, y: 5 };
        engine.execute_command(cmd1).unwrap();
        
        // Place a powerline over it
        let cmd2 = EngineCommand::PlaceTool { tool: Tool::PowerLine, x: 5, y: 5 };
